...
def main():
    ...
    parser.add_argument("--generate_action_histogram", action='store_true', dest='generate_action_histogram')
...
#    else:
    ...
    elapsed_times = []
    if args.generate_action_histogram:
        all_actions = [[] for i in range(4)]
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(-1, max_steps - start_step):
            ...
            if args.generate_action_histogram:
                ...
                numpy_actions = inputs['actions'].eval()
                print("numpy actions:", numpy_actions)
                for i in range(numpy_actions.shape[-1]):
                    action_in_dim = numpy_actions[:, :, i].reshape(-1)
                    print(action_in_dim.shape)
                    all_actions[i].extend(action_in_dim.tolist())
                if step > 10:
                    import matplotlib.pyplot as plt
                    num_bins = 100

                    for i in range(len(all_actions)):
                        acts = np.array(all_actions[i])
                        plt.hist(acts, num_bins, facecolor='blue', log=True, alpha=0.5)
                        plt.savefig('action_hists/actions_{}.png'.format(i))
                        plt.clf()
