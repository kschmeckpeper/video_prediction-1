...
#    inputs['actions'] = tf.Print(inputs['actions'], [inputs['actions'][:, :hparams.num_supervised, :]], "First nine", summarize=-1)
    reshaped_actions = tf.reshape(inputs['actions'][:, :hparams.num_supervised, :], [-1, inputs['actions'].shape[-1]])
    if hparams.action_encoder_norm_layer is not None:
        norm_layer = ops.get_norm_layer(hparams.action_encoder_norm_layer)
...
class DNACell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, inputs, hparams, reuse=None):
        ...
        if self.hparams.n_gpus == 1:
            self.encoder_gpu = 'gpu:0'
            self.decoder_gpu = 'gpu:0'
        elif self.hparams.n_gpus == 2:
            self.encoder_gpu = 'gpu:0'
            self.decoder_gpu = 'gpu:1'
        else:
            raise NotImplementedError("Only 1 or 2 gpus are currently supported")
    ...
    def call(self, inputs, states):
        ...
        layers = []
        new_conv_rnn_states = []
        with tf.device(self.encoder_gpu):
        ...
        with tf.device(self.decoder_gpu):
...
def generator_fn(inputs, mode, outputs_enc=None, hparams=None):
    ...
    if hparams.use_encoded_actions:
        ...
        if mode == 'test':
            ...
            eps = tf.random_normal([hparams.sequence_length - 1, hparams.num_supervised, hparams.encoded_action_size], 0, 1)
...
#        decoded_actions['decoded_actions'] = action_probs['action_mu']
    ...
    if not hparams.train_with_partial_actions:
        hparams.batch_size = hparams.num_supervised
        ...
        # The futures of the supervised images are predicted twice, one from the true actions
        # and once from the actions generated by the inverse model
        print("batch:size:", hparams.batch_size)
        print("num_sup:", hparams.num_supervised)
        hparams.batch_size = hparams.batch_size + hparams.num_supervised
        print("modified batch_size:", hparams.batch_size)
        inputs['images'] = tf.concat([inputs['images'], inputs['images'][:, :hparams.num_supervised, :, :, :]], axis=1)
        inputs['use_action'] = tf.concat([inputs['use_action'], inputs['use_action'][:, :hparams.num_supervised, :]], axis=1)
        inputs['actions'] = tf.concat([inputs['actions'], inputs['actions'][:, :hparams.num_supervised, :]], axis=1)
        inputs['encoded_actions'] = tf.concat([inputs['encoded_actions'], additional_encoded_actions['encoded_actions_inverse'][:, :hparams.num_supervised, :]], axis=1)
...
class SAVPVideoPredictionModel(VideoPredictionModel):
    ...
    def generator_loss_fn(self, inputs, outputs, targets):
        if not self.hparams.train_with_partial_actions:
            targets = targets[:, :self.hparams.num_supervised, :, :, :]
